...
public class SSLJedisClusterTest extends Assert {
  ...
  private HostAndPort sslNodeInfo1 = HostAndPortUtil.getSslClusterServers().get(0);
  private HostAndPort sslNodeInfo2 = HostAndPortUtil.getSslClusterServers().get(1);
  private HostAndPort sslNodeInfo3 = HostAndPortUtil.getSslClusterServers().get(2);
  private HostAndPort sslNodeInfo4 = HostAndPortUtil.getSslClusterServers().get(3);
  private HostAndPort sslNodeInfoSlave2 = HostAndPortUtil.getSslClusterServers().get(4);
  ...
  public void setUp() throws InterruptedException {
    ...
    node1 = new Jedis(sslNodeInfo1.getHost(), sslNodeInfo1.getPort(), sslNodeInfo1.isSsl());
    node1.connect();
    node1.flushAll();

    node2 = new Jedis(sslNodeInfo2.getHost(), sslNodeInfo2.getPort(), sslNodeInfo2.isSsl());
    node2.connect();
    node2.flushAll();

    node3 = new Jedis(sslNodeInfo3.getHost(), sslNodeInfo3.getPort(), sslNodeInfo3.isSsl());
    node3.connect();
    node3.flushAll();

    node4 = new Jedis(sslNodeInfo4.getHost(), sslNodeInfo4.getPort(), sslNodeInfo4.isSsl());
    node4.connect();
    node4.flushAll();

    nodeSlave2 = new Jedis(sslNodeInfoSlave2.getHost(), sslNodeInfoSlave2.getPort(), sslNodeInfoSlave2.isSsl());
    nodeSlave2.connect();
    nodeSlave2.flushAll();
    // ---- configure cluster
